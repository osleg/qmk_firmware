...
enum mode { INSERT_MODE, INSERT_SAVE_MODE, COMMAND_MODE, REPLACE_MODE_INIT, REPLACE_MODE, REPEAT_MODE };
...
void add_to_vim_cmd(char c) {
    ...
    if (currcmdsize == CMDBUFFSIZE - 1) go_insert_mode();
}

void tap_code_num(uint16_t keycode, int num) {
    for (int i = 0; i < num; i++) tap_code(keycode);
}
...
void mod_type(uint16_t modcode, uint16_t keycode) { mod_type_num(modcode, keycode, 1); }
    ...
    switch (last_char) {
        ...
        case 'G':
            if (prev_char == 'd' || prev_char == 'x' || prev_char == 'X' || prev_char == 'c' || prev_char == 'y') {
                ...
                UNHOLD_SHIFT;
                if (prev_char == 'c' || prev_char == 'd') tap_code(KC_DEL);
                if (prev_char == 'c') go_insert_mode();
                if (prev_char == 'y') {
                    mod_type(GUI, KC_C);
                    tap_code(KC_LEFT);
        ...
        case 'g':
            if (prev_char == 'g') {
                ...
                if (ch2 == 'd' || ch2 == 'x' || ch2 == 'X' || ch2 == 'c' || ch2 == 'y') {
                    ...
                    mod_type(GUI, KC_UP);
                    UNHOLD_SHIFT;
                    if (ch2 == 'c' || ch2 == 'd') tap_code(KC_DEL);
                    if (ch2 == 'c') go_insert_mode();
                    if (ch2 == 'y') {
                        mod_type(GUI, KC_C);
                        tap_code(KC_RIGHT);
                        tap_code(KC_LEFT);
...
// clang-format off
const char keycode_to_char_map[58] = {NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR, 'a',
  'b', 'c', 'd', 'e', NO_CHAR, 'g', 'h', 'i', 'j', 'k', 'l', NO_CHAR, NO_CHAR,
  'o', 'p', 'q', 'r', NO_CHAR, NO_CHAR, 'u', 'v', 'w', 'x', 'y', NO_CHAR, '1',
  '2', '3', '4', '5', '6', '7', '8', '9', '0', NO_CHAR, NO_CHAR, NO_CHAR,
  NO_CHAR, ' ', NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR, '\\', NO_CHAR, NO_CHAR,
  NO_CHAR, NO_CHAR, NO_CHAR, '.', '/'};

const char keycode_to_char_map_shifted[58] = {NO_CHAR, NO_CHAR, NO_CHAR,
  NO_CHAR, 'A', NO_CHAR, 'C', 'D', NO_CHAR, NO_CHAR, 'G', NO_CHAR, 'I',
  NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR, 'O', 'P', NO_CHAR, NO_CHAR,
  NO_CHAR, NO_CHAR, 'U', 'V', NO_CHAR, 'X', NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR,
  NO_CHAR, '$', NO_CHAR, '^', NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR, NO_CHAR,
...
char keycode_to_char(uint16_t keycode, keyrecord_t *record) {
    ...
    if (keycode < KC_A || keycode > KC_SLASH) return NO_CHAR;
...
void handle_vim_cmd(void) {
    ...
    if (handle_cmd(last_char, prev_char, prev_num)) currcmdsize = 0;
...
bool handle_vim_mode(uint16_t keycode, keyrecord_t *record, uint8_t vim_layer_no, uint8_t base_layer_no) {
    ...
    if (!record->event.pressed) return currmode != INSERT_MODE && currmode != INSERT_SAVE_MODE;

    vim_layer = vim_layer_no;

    if (keycode == VIM_ESC) {
        currmode = COMMAND_MODE;
        layer_off(base_layer_no);
        layer_on(vim_layer);
        visual_mode = false;
        currcmdsize = 0;
    }
